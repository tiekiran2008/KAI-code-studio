      ...
      <div className="glass-panel bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Shield className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-gray-500" />
              <select
                value={role}
                onChange={(e) => setRole(e.target.value as RoleEnum)}
                className="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-gray-950/50 border border-slate-300 dark:border-gray-700/50 rounded-lg text-sm text-slate-900 dark:text-gray-200 focus:outline-none focus:border-indigo-500/50 appearance-none"
              >
                {roles.map(r => (
          ...
          <div className="pt-4 flex gap-3 justify-end">
            ...
            <button
              ...
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-sm font-medium transition-colors shadow-lg shadow-indigo-600/20"
            >
